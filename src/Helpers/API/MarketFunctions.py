...
def getCurrentBinancePrice(pair: Pair, requests=requests, time=time, timeout_limit: int=3):
    """
    ...
    if timeout_limit == 0:
        raise Exception("Timeout limit exceeded")  

    time.sleep(5)
    try:
        print("TRYING --> ", timeout_limit  )
        req = requests.get(f"https://api.binance.com/api/v1/ticker/price?symbol={pair.value}")
        print("response from requests get ---> ", req)
        
    except Exception as e:
        time.sleep(15)
        timeout_limit -= 1 
        getCurrentBinancePrice(pair, requests=requests, time=time, timeout_limit=timeout_limit)
